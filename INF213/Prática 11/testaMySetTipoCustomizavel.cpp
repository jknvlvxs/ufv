...
public: 
	...
	bool operator>(const Aluno &first){
		if(first.getNome() > nome) return true;
		else if (first.getNome() < nome) return false;
		
		if(first.getMatricula() > matricula) return true;
		else if (first.getMatricula() < matricula) return false;
