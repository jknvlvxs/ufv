...
void mergeSort(int *v, int p, int r, int *aux) {
    for(int tam_sub = 1; tam_sub < r; tam_sub *= 2){
        for(int beg = 0; beg < r; beg += 2 * tam_sub){
            int mid = beg + tam_sub < r ? beg + tam_sub : r;
            int end = mid + tam_sub < r ? mid + tam_sub : r;
            merge(v, beg, mid, end, aux);
        }
    }
}
