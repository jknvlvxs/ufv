...
private:
	...
	Node<T> * copiaDados(Node<T>* parent, const Node<T>* other) const;
...
//TODO
template <class T>
Node<T> * MySet<T>::copiaDados(Node<T> *parent, const Node<T> *other) const {
	if(other == NULL) return NULL;
	
	Node<T> *ans = new Node<T>(other->elem);
	ans->parent = parent;
	ans->left = copiaDados(ans, other->left);
	ans->right = copiaDados(ans, other->right);
...
MySet<T> & MySet<T>::operator=(const MySet &other) {
	...
	destroy(this->root);
	root = copiaDados(NULL, other.root);
	size_ = other.size_;
	return *this;
}
