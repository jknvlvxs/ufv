...
/*
A complexidade do pior caso da função é O(n^2). Pois é necessário percorrer o vetor 2 vezes o seu tamanho.
Uma para encontrar os elementos repetidos, outra para removê-los do array.
*/
template<class T>
int MyVec<T>::eraseMatchingElements(const T&elem) {
	int numElementosRemovidos = 0;
	int index = 0;
	...
	for(int i = 0; i < size(); i++){
		if(data[i] != elem){
			...
			numElementosRemovidos++;
		}
	}
	dataSize = dataSize - numElementosRemovidos;

	return numElementosRemovidos;
}
