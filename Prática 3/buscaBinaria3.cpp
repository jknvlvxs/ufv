...
int buscaBin(int *array,int begin, int end, int chave) {
	...
	if (begin <= end && array[begin] >= chave) return begin; // caso não possua o numero, porém, há um numero maior
	int meio = (end-begin)/2 + begin;
	if (array[meio] == chave) { 
		if(array[meio-1] != chave) return meio; // verificação para numeros repetidos
		else return buscaBin(array,begin, meio-1, chave); 
	}
	if (array[meio] > chave) {
		if(array[meio-1] < chave) return meio;
		else return buscaBin(array,begin, meio-1, chave); 
	}
